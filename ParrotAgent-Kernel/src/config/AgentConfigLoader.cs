...
namespace ParrotAgent.Kernel
{
    ...
    /// 
    /// </summary>
    internal class AgentConfigLoader
    {
        public const string EnvVar = "PARROTAGENT_CONFIG";
        public const string CwdFileName = ".parrotagent.yaml";
        public const string UserDirName = ".parrotagent";
        ...
        /// </summary>
        public static AgentConfig Load() => Load(explicitPath: null);
        ...
        private static string? ResolvePath(string? explicitPath)
        {
            ...
            if (explicitPath is not null)
            {
                return File.Exists(explicitPath) ? explicitPath
                                                 : throw new AgentConfigException($"指定的配置文件不存在: {explicitPath}", explicitPath);
            }

            // 优先级 1：环境变量 PARROTAGENT_CONFIG（明确意图，不存在则报错）
            var envPath = Environment.GetEnvironmentVariable(EnvVar);
            if (!string.IsNullOrWhiteSpace(envPath))
            {
                return File.Exists(envPath) ? envPath
                                            : throw new AgentConfigException($"环境变量 {EnvVar} 指向的文件不存在: {envPath}", envPath);
            }

            // 优先级 2：当前工作目录 .parrotagent.yaml
            var cwdPath = Path.Combine(Environment.CurrentDirectory, CwdFileName);
            if (File.Exists(cwdPath)) return cwdPath;

            // 优先级 3：用户目录 ~/.parrotagent/config.yaml
        ...
        // —— YAML 解析 + 行号捕获 ——
        private static AgentConfig Parse(string path)
        {
            var text = File.ReadAllText(path);
            if (string.IsNullOrWhiteSpace(text))
                throw new AgentConfigException("配置文件为空", path);
            ...
            try
            {
                var deserializer = new DeserializerBuilder()
                    ...
                    .Build();
                return deserializer.Deserialize<AgentConfig>(text) ?? throw new AgentConfigException("配置文件内容为 null", path);
            }
            catch (YamlException ex)
            {
                throw new AgentConfigException($"YAML 解析失败: {ex.Message}", path, (int)ex.Start.Line, (int)ex.Start.Column, ex);
            }
        }

        // —— ${VAR} 环境变量展开（所有 ProviderConfig 字符串字段）——
        private static AgentConfig ExpandEnv(AgentConfig config, string sourcePath)
        {
        ...
        private static string ExpandField(string value, string fieldPath, string sourcePath)
        {
            ...
            return EnvVarPattern.Replace(value, m =>
            {
                ...
                if (string.IsNullOrEmpty(v))
                    throw new AgentConfigException($"环境变量 '{name}' 未设置（引用于 {fieldPath}）", sourcePath);
        ...
        // —— 语义校验 ——
        private static AgentConfig Validate(AgentConfig config, string? sourcePath)
        {
            if (config.Providers.Count == 0)
                throw new AgentConfigException("providers 不能为空", sourcePath);
            ...
            for (var i = 0; i < config.Providers.Count; i++)
            {
                ...
                if (string.IsNullOrWhiteSpace(p.Name))
                    throw new AgentConfigException($"{prefix}.name 不能为空", sourcePath);
                if (string.IsNullOrWhiteSpace(p.Protocol))
                    throw new AgentConfigException($"{prefix}.protocol 不能为空", sourcePath);
                if (!SupportedProtocols.Contains(p.Protocol))
                    throw new AgentConfigException($"{prefix}.protocol '{p.Protocol}' 不支持（允许: mock/openai/anthropic）", sourcePath);
            }
            ...
            if (duplicate is not null)
            {
                ...
                throw new AgentConfigException($"providers 名称重复: '{duplicate.Key}' (providers[{indices[0]}] 与 providers[{indices[1]}])",sourcePath);
            }
            ...
            {
                throw new AgentConfigException($"active_provider '{config.ActiveProvider}' 未在 providers 中定义", sourcePath);
            }
        ...
        /// </summary>
        /// <returns></returns>
        private static AgentConfig Default() => new()
        {
            ActiveProvider = "mock",
            Providers = new[]
            {
