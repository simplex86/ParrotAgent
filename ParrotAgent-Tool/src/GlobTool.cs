...
namespace ParrotAgent.Tool.src
{
    ...
    public sealed class GlobTool : ITool
    {
        ...
        {
            var pattern = input.GetRequiredString("pattern", out var err1);
            if (err1 is not null) return ToolResult.Fail(err1);
            var path = input.GetOptionalString("path", out var err2, ".");
            if (err2 is not null) return ToolResult.Fail(err2);

            if (string.IsNullOrWhiteSpace(pattern)) return ToolResult.Fail("参数 pattern 不能为空");
            if (!Directory.Exists(path)) return ToolResult.Fail($"目录不存在：{path}");
            ...
            try
            {
                ...
                return ToolResult.Ok(matches.Count == 0 ? "未找到匹配文件"
