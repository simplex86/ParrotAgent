...
namespace ParrotAgent.Tool
{
    ...
    public sealed class EditFileTool : ITool
    {
        ...
        {
            var path = input.GetRequiredString("path", out var err1);
            if (err1 is not null) return ToolResult.Fail(err1);
            var oldText = input.GetRequiredString("old_text", out var err2);
            if (err2 is not null) return ToolResult.Fail(err2);
            var newText = input.GetRequiredString("new_text", out var err3);
            if (err3 is not null) return ToolResult.Fail(err3);

            if (string.IsNullOrWhiteSpace(path)) return ToolResult.Fail("参数 path 不能为空");
            if (oldText.Length == 0) return ToolResult.Fail("参数 old_text 不能为空（如需清空文件请用 write_file）");

            // 先检查目录（File.Exists 对目录返回 false，会误报"文件不存在"）
            if (Directory.Exists(path)) return ToolResult.Fail($"路径是目录而非文件：{path}");
